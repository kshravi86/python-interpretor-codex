<#!
.SYNOPSIS
  Fully uninstall iTunes and all related Apple components from Windows.

.DESCRIPTION
  Removes in Apple-recommended order:
    1) iTunes
  ...
  Also stops Apple services, deletes residual folders, registry keys, scheduled tasks,
  and firewall rules commonly installed by iTunes.
...
.NOTES
  Run as Administrator. A reboot is recommended after removal.
#>
...
function Stop-AppleServices {
  $svcNames = @('Apple Mobile Device Service','Bonjour Service','iPod Service')
...
if (-not $Force) {
  $res = Read-Host 'This will fully remove iTunes and related Apple components. Continue? (y/N)'
...
# Uninstall in Apple-recommended order
$order = @(
  '(?i)^iTunes$','(?i)^Apple Software Update$','(?i)^Apple Mobile Device Support$','(?i)^Bonjour$','(?i)^Apple Application Support \(64-bit\)$','(?i)^Apple Application Support \(32-bit\)$'
)

foreach ($pattern in $order) {
  $entries = Get-UninstallEntries -matchNames @($pattern)
  foreach ($e in $entries) {
    if ($e.UninstallString) {
      Write-Host "Uninstalling: $($e.DisplayName)" -ForegroundColor Cyan
      Invoke-Uninstall -displayName $e.DisplayName -uninstallString $e.UninstallString | Out-Null
    }
  }
}
...
$paths = @(
  ...
  '$env:ProgramFiles\Apple Software Update',
  ...
  '$env:APPDATA\Apple Computer\Mobile Sync',
  '$env:APPDATA\Apple Computer\Preferences'
)
...
Remove-ScheduledTasks -namePatterns @('(?i)Apple','(?i)iTunes','(?i)Bonjour','(?i)AppleSoftwareUpdate')
Remove-FirewallRules -namePatterns @('(?i)Apple','(?i)iTunes','(?i)Bonjour')

Write-Host 'Done. Please reboot to finalize the removal.' -ForegroundColor Green
